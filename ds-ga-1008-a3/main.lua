...
local params = {
                ...
                layers=2,
                decay=1.5,
                rnn_size=400, -- hidden unit size
                dropout=0.3, 
                init_weight=0.1, -- random weight initialization limits
                lr=1, --learning rate
                vocab_size=10000, -- limit on the vocabulary size
                max_epoch=10,  -- when to start decaying learning rate
                max_max_epoch=30, -- final epoch
                max_grad_norm=10,-- clip when gradients exceed this norm value
				model = 'lstm', -- lstm/gru
				save = 'logs_gpu' -- path to save model and results]
				gpu = true 
               }
...
function setup_GRU()
    ...
    local core_network = create_network_gru()
...
function reset_state_gru(state)
    state.pos = 1
...
function fp(state)
    ...
    if state.pos + params.seq_length > state.data:size(1) then
        if params.model =='lstm' then
        reset_state(state)
        else reset_state_gru(state)
        end
    end
...
function bp_gru(state)
    ...
    for i = params.seq_length, 1, -1 do
        ...
        local pred_err = transfer_data(torch.zeros(params.batch_size, params.vocab_size))
        -- tmp stores the ds
        local tmp = model.rnns[i]:backward({x, y, s},
                                           {derr, model.ds,pred_err})[3]
        -- remember (to, from)
...
function run_valid()
    ...
    else  reset_state_gru(state_valid)
    end
    ...
    valLogger:add{g_f3(torch.exp(perp / len))}
...
function run_test()
    ...
    else reset_state_gru(state_test)
    end
...
paths.mkdir(params.save)
trainLogger = optim.Logger(paths.concat(params.save, 'train.log'))
trainLogger:setNames{'% train perp', '% dw'}
trainLogger.showPlot = false

valLogger = optim.Logger(paths.concat(params.save, 'val.log'))
valLogger:setNames{'% val perp'}
...
while epoch < params.max_max_epoch do
    ...
    if step % torch.round(epoch_size / 10) == 10 then
        ...
        trainLogger:add{g_f3(torch.exp(perps:mean())), g_f3(model.norm_dw)}
